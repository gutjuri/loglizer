...
def load_linux(x_train_file, x_test_file, x_val_file, label_file=None, window='session', train_ratio=0.5, split_type='sequential', save_csv=False, window_size=0):
    df_train = readLinuxLog(x_train_file)
    df_test = readLinuxLog(x_test_file)
    df_val = readLinuxLog(x_val_file)
    ...
    #print(label_dict)
    df_train['label'] = df_train['EventId'].apply(
        lambda x: 1 if label_dict[x] == 'Anomaly' else 0)
    df_test['label'] = df_test['EventId'].apply(
        lambda x: 1 if label_dict[x] == 'Anomaly' else 0)
    df_val['label'] = df_val['EventId'].apply(
        lambda x: 1 if label_dict[x] == 'Anomaly' else 0)
    ...
    x_train = df_train['EventSequence'].values
    x_test = df_test['EventSequence'].values
    y_test = df_test['label'].values
    x_val = df_val['EventSequence'].values
    ...
    print('Test: {} instances, {} anomaly, {} normal'
          .format(y_test.shape[0], y_test.sum(), y_test.shape[0] - y_test.sum()))
    print('Val: {} instances, {} anomaly, {} normal\n'
          .format(y_val.shape[0], y_val.sum(), y_val.shape[0] - y_val.sum()))
